...
Promise.all([loadImage('stage/001.png').then(i=>assets.background=i),loadImage('atk/001.png').then(i=>assets.bullet=i),loadImage('atk/002.png').then(i=>assets.lightning=i),...Array.from({length:32},(_,n)=>loadImage(`denden/${String(n+1).padStart(3,'0')}.png`).then(i=>sprites[n]=i)),...Array.from({length:32},(_,n)=>loadImage(`skill/${String(n+1).padStart(3,'0')}.png`).then(i=>skillSprites[n]=i))]).then(()=>{state='ready';$('start').disabled=false;$('start').textContent='START RUN →';$('load-status').textContent='AREA 1-1 • 草原 / 約1〜2分';reset();}).catch(e=>{$('modal-title').textContent='画像を読み込めませんでした';$('modal-copy').textContent=`${e.message} を確認してください。`;$('load-status').textContent='ページを再読み込みしてください';});
function reset(){
...
for(const el of skillButtons){const index=Number(el.dataset.skill);
  el.addEventListener('pointerdown',e=>{e.preventDefault();if(state!=='playing')return;el.setPointerCapture(e.pointerId);if(index===0)beginCharge(`pointer:${e.pointerId}`);else if(index===1)castThunder();else{hintTimer=1.5;$('hint').textContent='SKILL 3 は準備中';}});
...
window.addEventListener('keydown',e=>{if(!['Digit1','Digit2'].includes(e.code))return;e.preventDefault();if(e.repeat||state!=='playing')return;if(e.code==='Digit1')beginCharge('keyboard');else castThunder();});
...
function updateSkillHUD(){for(let i=0;i<2;i++){const b=skillButtons[i],cd=skillState.cooldowns[i],charging=i===0&&skillState.charging;
...
function tick(dt){
  ...
  const locked=thunderLocked();if(locked){jumpRequest=0;p.knock=0;}
  const move=!locked&&magnitude>.35?Math.sign(a):0;p.vx=move*(running?CONFIG.dashSpeed:CONFIG.walkSpeed);if(move)p.dir=move;
  ...
  if(!locked&&(pointers.shoot.size||keys.has('KeyJ'))&&shootClock<=0)fire();
  ...
  if(state==='playing')updateSkills(dt);
...
function drawSkills(){const s=skillState,p=player;
