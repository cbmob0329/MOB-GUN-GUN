...
function beginCharge(source){if(state!=='playing'||selectedCharacter!=='denden'||thunderLocked()||thunderBullet||skillState.charging||skillState.cooldowns[0]>0)return;skillState.charging=true;skillState.charge=0;skillButtons[0].classList.add('pressed');}
...
function castThunder(){if(state!=='playing'||skillState.cooldowns[1]>0||skillState.charging||thunderBullet)return;chargeSources.clear();skillState.charging=false;skillState.charge=0;skillButtons[0].classList.remove('pressed');player.vx=0;player.knock=0;jumpRequest=0;skillState.thunderLeft=CONFIG.skills.thunder.duration;skillState.thunderAge=0;skillState.waveClock=0;skillState.wave=0;skillState.cooldowns[1]=skillCooldown('thunder');startGiantThunder();}
function hitEnemy(e,damage,dir=1){if(e.type==='crate'){damageCrate(e,damage);return;}if(e.death>=0)return;if(e.type==='miramob'&&miraSpecial(e.action))damage=Math.max(1,Math.ceil(damage*.5));e.hp=Math.max(0,e.hp-damage);e.flash=.1;e.knock=dir*110;burst(e.x,e.y-e.h/2);if(e.hp===0){e.death=0;kills++;burst(e.x,e.y-e.h/2,12);}}
