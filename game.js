...
const CONFIG = Object.freeze({
  ...
  enemies:{miramob:{name:'ミラモブ',hp:960,damage:6,speed:95,color:'#a756d1'},miira:{name:'モブミイラ',hp:24,damage:2,speed:42,color:'#d6c49d'},patrol:{name:'クサマル',hp:24,damage:6,speed:56,color:'#8cae4b',sprite:null},
...
function tick(dt){
  ...
  if(jumpRequest>0&&!locked&&(p.coyote>0||(selectedCharacter!=='nyoro'&&p.jumpsUsed<2))){p.jumpsUsed=p.coyote>0?1:Math.max(1,p.jumpsUsed)+1;p.vy=selectedCharacter==='nyoro'?-NYORO.jumpForce:-CONFIG.jumpForce;p.grounded=false;p.coyote=0;p.jumpAge=0;jumpRequest=0;burst(p.x,p.y-4,p.jumpsUsed===2?10:5,'#d6e7a4');}
