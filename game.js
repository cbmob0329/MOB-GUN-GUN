...
function reset(){
  ...
  addEnemy('tank',21200);resetWorld();resetSupport();hintTimer=0;updateHUD();
}
...
window.addEventListener('keydown',e=>{if(['ArrowLeft','ArrowRight','ArrowUp','Space','KeyA','KeyD','KeyJ','KeyK','ShiftLeft','ShiftRight','Escape'].includes(e.code)){e.preventDefault();if(e.code==='Escape'&&!e.repeat){state==='paused'?resume():pause();return;}if(state!=='playing')return;keys.add(e.code);if(e.code==='KeyJ'&&!e.repeat){tetsuAttack();nyoroAttack();}if(['Space','ArrowUp','KeyK'].includes(e.code)&&!e.repeat)jumpRequest=CONFIG.jumpBuffer;}});
window.addEventListener('keyup',e=>keys.delete(e.code));
for(const type of ['shoot','jump']){const el=$(type);el.addEventListener('pointerdown',e=>{e.preventDefault();if(state!=='playing')return;el.setPointerCapture(e.pointerId);pointers[type].add(e.pointerId);el.classList.add('pressed');if(type==='shoot'){tetsuAttack();nyoroAttack();}if(type==='jump')jumpRequest=CONFIG.jumpBuffer;});const release=e=>{pointers[type].delete(e.pointerId);if(!pointers[type].size)el.classList.remove('pressed');};for(const event of ['pointerup','pointercancel','lostpointercapture'])el.addEventListener(event,release);}
...
for(const el of skillButtons){const index=Number(el.dataset.skill);
  el.addEventListener('pointerdown',e=>{e.preventDefault();if(state!=='playing')return;el.setPointerCapture(e.pointerId);if(selectedCharacter==='nyoro'){castNyoro(index);return;}if(selectedCharacter==='tetsu'){castTetsu(index);return;}if(index===0)beginCharge(`pointer:${e.pointerId}`);else if(index===1)castThunder();else castThunderBullet();});
...
window.addEventListener('keydown',e=>{if(!['Digit1','Digit2','Digit3'].includes(e.code))return;e.preventDefault();if(e.repeat||state!=='playing')return;if(selectedCharacter==='nyoro'){castNyoro(Number(e.code.slice(-1))-1);return;}if(selectedCharacter==='tetsu'){castTetsu(Number(e.code.slice(-1))-1);return;}if(e.code==='Digit1')beginCharge('keyboard');else if(e.code==='Digit2')castThunder();else castThunderBullet();});
...
function tick(dt){
  ...
  if(state==='playing'){updateThunderBullet(dt);updateDirtBalls(dt);if(selectedCharacter==='tetsu')updateTetsu(dt);else if(selectedCharacter==='denden')updateSkills(dt);updateNyoro(dt);updateBombs(dt);}
  ...
  if(state==='playing'){updateWorld(dt);updatePink(dt);}
  updateHUD();
}
...
function drawSkills(){drawNyoroEffects();drawBombs();if(selectedCharacter==='nyoro')return;if(selectedCharacter==='tetsu'){drawTetsuEffects();return;}const s=skillState,p=player;
...
window.addEventListener('keydown',e=>{if(e.code==='KeyR'&&!e.repeat){e.preventDefault();commandPink();}});
