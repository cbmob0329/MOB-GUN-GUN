...
const CONFIG = Object.freeze({
  ...
  shootInterval:0.20, magazineSize:8, reloadDuration:1, bulletSpeed:1050, bulletDamage:12, maxHP:50,
  playerHeight:88, playerColliderWidth:34, playerColliderHeight:65,
  invincibility:1.25, coyoteTime:0.10, jumpBuffer:0.13,
  skills:{trick:{maxCharge:.25,minRadius:18,maxRadius:58,minMultiplier:10,maxMultiplier:10,cooldown:8,minCooldown:5,cooldownReduction:0,speed:850,splashRatio:.6,splashRadius:260},
    thunder:{duration:2.5,lockDuration:2,multiplier:5,cooldown:12,minCooldown:9,cooldownReduction:0,minInterval:.025,maxInterval:.075,fallSpeed:1800}},
  enemies:{miramob:{name:'ミラモブ',hp:1320,damage:6,speed:95,color:'#a756d1'},miira:{name:'モブミイラ',hp:24,damage:2,speed:42,color:'#d6c49d'},patrol:{name:'クサマル',hp:24,damage:6,speed:56,color:'#8cae4b',sprite:null},
...
Promise.all([loadTetsu(),loadMiira(),loadSupport(),loadNyoro(),loadMira(),loadGiantThunder(),loadImage('stage/001.png').then(i=>assets.background=i),loadImage('atk/001.png').then(i=>assets.bullet=i),loadImage('atk/002.png').then(i=>assets.lightning=i),...Array.from({length:32},(_,n)=>loadImage(`denden/${String(n+1).padStart(3,'0')}.png`).then(i=>sprites[n]=i)),...Array.from({length:32},(_,n)=>loadImage(`skill/${String(n+1).padStart(3,'0')}.png`).then(i=>skillSprites[n]=i))]).then(()=>{state='ready';$('start').disabled=false;$('start').textContent='START RUN →';$('load-status').textContent='AREA 1-1 • 草原 / BOSS ミラモブ';reset();updateCharacterUI();}).catch(e=>{$('modal-title').textContent='画像を読み込めませんでした';$('modal-copy').textContent=`${e.message} を確認してください。`;$('load-status').textContent='ページを再読み込みしてください';});
function reset(){
  resetTetsu();resetNyoro();bombBursts=[];dirtBalls=[];
...
window.addEventListener('keydown',e=>{if(e.target instanceof Element&&e.target.closest('input,textarea,select'))return;if(['ArrowLeft','ArrowRight','ArrowUp','Space','KeyA','KeyD','KeyJ','KeyK','ShiftLeft','ShiftRight','Escape'].includes(e.code)){e.preventDefault();if(e.code==='Escape'&&!e.repeat){state==='paused'?resume():pause();return;}if(state!=='playing'||bossIntro())return;keys.add(e.code);if(e.code==='KeyJ'&&!e.repeat){tetsuAttack();nyoroAttack();miraAttack();}if(['Space','ArrowUp','KeyK'].includes(e.code)&&!e.repeat)jumpRequest=CONFIG.jumpBuffer;}});
window.addEventListener('keyup',e=>keys.delete(e.code));
for(const type of ['shoot','jump']){const el=$(type);el.addEventListener('pointerdown',e=>{e.preventDefault();if(state!=='playing'||bossIntro())return;el.setPointerCapture(e.pointerId);pointers[type].add(e.pointerId);el.classList.add('pressed');if(type==='shoot'){tetsuAttack();nyoroAttack();miraAttack();}if(type==='jump')jumpRequest=CONFIG.jumpBuffer;});const release=e=>{pointers[type].delete(e.pointerId);if(!pointers[type].size)el.classList.remove('pressed');};for(const event of ['pointerup','pointercancel','lostpointercapture'])el.addEventListener(event,release);}
...
for(const el of skillButtons){const index=Number(el.dataset.skill);
  el.addEventListener('pointerdown',e=>{e.preventDefault();if(state!=='playing'||bossIntro())return;el.setPointerCapture(e.pointerId);if(selectedCharacter==='miramob'){castMira(index);return;}if(selectedCharacter==='nyoro'){castNyoro(index);return;}if(selectedCharacter==='tetsu'){castTetsu(index);return;}if(index===0)beginCharge(`pointer:${e.pointerId}`);else if(index===1)castThunder();else castThunderBullet();});
...
window.addEventListener('keydown',e=>{if(e.target instanceof Element&&e.target.closest('input,textarea,select'))return;if(!['Digit1','Digit2','Digit3'].includes(e.code))return;e.preventDefault();if(e.repeat||state!=='playing'||bossIntro())return;if(selectedCharacter==='miramob'){castMira(Number(e.code.slice(-1))-1);return;}if(selectedCharacter==='nyoro'){castNyoro(Number(e.code.slice(-1))-1);return;}if(selectedCharacter==='tetsu'){castTetsu(Number(e.code.slice(-1))-1);return;}if(e.code==='Digit1')beginCharge('keyboard');else if(e.code==='Digit2')castThunder();else castThunderBullet();});
...
function beginCharge(source){if(state!=='playing'||bossIntro()||selectedCharacter!=='denden'||thunderLocked()||thunderBullet||skillState.charging||skillState.cooldowns[0]>0)return;skillState.charging=true;skillState.charge=0;skillButtons[0].classList.add('pressed');}
function endCharge(){/* One-tap automatic charge; releasing does not cancel. */}
...
function castThunder(){if(state!=='playing'||bossIntro()||skillState.cooldowns[1]>0||skillState.charging||thunderBullet)return;chargeSources.clear();skillState.charging=false;skillState.charge=0;skillButtons[0].classList.remove('pressed');player.vx=0;player.knock=0;jumpRequest=0;skillState.thunderLeft=CONFIG.skills.thunder.duration;skillState.thunderAge=0;skillState.waveClock=0;skillState.wave=0;skillState.cooldowns[1]=skillCooldown('thunder');startGiantThunder();}
function hitEnemy(e,damage,dir=1){if(e.type==='crate'){damageCrate(e,damage);return;}if(e.death>=0)return;if(e.type==='miramob'&&miraSpecial(e.action))damage=Math.max(1,Math.ceil(damage*.5));e.hp=Math.max(0,e.hp-damage);e.flash=.1;e.knock=dir*110;burst(e.x,e.y-e.h/2);if(e.hp===0){e.death=0;kills++;dropBossDorayaki(e);burst(e.x,e.y-e.h/2,12);}}
...
function updateSkills(dt){const s=skillState,c=CONFIG.skills.thunder;s.releaseAge+=dt;s.thunderAge+=dt;for(let i=0;i<2;i++)s.cooldowns[i]=Math.max(0,s.cooldowns[i]-dt);if(s.charging)s.charge=Math.min(CONFIG.skills.trick.maxCharge,s.charge+dt);if(s.charging&&s.charge>=CONFIG.skills.trick.maxCharge){releaseEnergy();s.charging=false;s.charge=0;skillButtons[0].classList.remove('pressed');}
  ...
  updateGiantThunder(dt);s.thunderLeft=giantThunder?Math.max(0,2-giantThunder.groundAge):0;for(const ex of explosions)ex.age+=dt;explosions=explosions.filter(ex=>ex.age<.65);
}
function updateSkillHUD(){if(selectedCharacter==='miramob'){skillButtons.forEach((b,i)=>{if(i>1)return;const cd=miraCooldowns[i];b.classList.toggle('cooling',cd>0);b.classList.remove('charging');b.setAttribute('aria-disabled',String(cd>0||!!miraAction));b.querySelector('small').textContent=cd>0?`${cd.toFixed(1)}s`:`0${i+1} READY`;});return;}if(selectedCharacter==='nyoro'){skillButtons.forEach((b,i)=>{const cd=nyoroCooldowns[i];b.classList.toggle('cooling',cd>0);b.classList.remove('charging');b.setAttribute('aria-disabled',String(cd>0||!!nyoroAction));b.querySelector('small').textContent=cd>0?`${cd.toFixed(1)}s`:`0${i+1} READY`;});return;}if(selectedCharacter==='tetsu'){skillButtons.forEach((b,i)=>{const cd=tetsuCooldowns[i];b.classList.toggle('cooling',cd>0);b.classList.remove('charging');b.setAttribute('aria-disabled',String(cd>0||!!tetsuAction));b.querySelector('small').textContent=cd>0?`${cd.toFixed(1)}s`:`0${i+1} READY`;});return;}for(let i=0;i<3;i++){const b=skillButtons[i],cd=i===2?thunderBulletCooldown:skillState.cooldowns[i],charging=i===0&&skillState.charging;
  b.classList.toggle('cooling',cd>0);b.classList.toggle('charging',charging);b.setAttribute('aria-disabled',String(cd>0));b.querySelector('small').textContent=charging?'AUTO':cd>0?`${cd.toFixed(1)}s`:`0${i+1} READY`;b.style.setProperty('--charge',`${charging?skillState.charge/CONFIG.skills.trick.maxCharge*100:0}%`);}}
...
function tick(dt){
  ...
  if(jumpRequest>0&&!locked&&(p.coyote>0||(selectedCharacter!=='nyoro'&&p.jumpsUsed<2))){p.jumpsUsed=p.coyote>0?1:Math.max(1,p.jumpsUsed)+1;p.vy=selectedCharacter==='nyoro'?-NYORO.jumpForce:selectedCharacter==='miramob'?-MIRA.jumpForce:-CONFIG.jumpForce;p.grounded=false;p.coyote=0;p.jumpAge=0;jumpRequest=0;burst(p.x,p.y-4,p.jumpsUsed===2?10:5,'#d6e7a4');}
  ...
  for(const e of enemies){e.stun=Math.max(0,(e.stun||0)-dt);e.electric=Math.max(0,(e.electric||0)-dt);e.flash=Math.max(0,e.flash-dt);if(e.dropping){e.dropping.vy+=1600*dt;e.y+=e.dropping.vy*dt;if(e.y>=CONFIG.groundY){e.y=CONFIG.groundY;e.dropping=null;burst(e.x,e.y,6,'#cbb693');}continue;}if(e.launch){const f=e.launch;e.x=clamp(e.x+f.vx*dt,24,CONFIG.worldWidth-24);e.y+=f.vy*dt;f.vy+=CONFIG.gravity*dt;f.vx*=Math.exp(-1.3*dt);f.angle+=f.spin*dt;if(e.death>=0&&e.type!=='miira')e.death+=dt;if(e.y>=f.floor&&f.vy>0){e.y=f.floor;e.home=e.x;e.launch=null;}continue;}if(e.death>=0){e.death+=dt;continue;}if(e.type==='miramob'){updateMiraBoss(e,dt);continue;}if(e.stun>0)continue;if(Math.abs(e.x-p.x)>1000)continue;if(e.type==='miira'){updateMiira(e,dt);continue;}const c=CONFIG.enemies[e.type];if(e.type==='chase'&&Math.abs(e.x-p.x)<470)e.dir=Math.sign(p.x-e.x)||e.dir;else if(e.x<e.home-e.range)e.dir=1;else if(e.x>e.home+e.range)e.dir=-1;e.x+=e.dir*c.speed*dt+e.knock*dt;e.knock*=Math.exp(-10*dt);e.x=clamp(e.x,e.home-e.range-80,e.home+e.range+80);if(Math.abs(p.x-e.x)<half+e.w*.42&&p.y>e.y-e.h&&p.y-CONFIG.playerColliderHeight<e.y)damagePlayer(e);}
  for(const b of bullets){const previous=b.x;b.x+=b.dir*CONFIG.bulletSpeed*dt;b.life-=dt;if(b.bomb&&b.life<=0)explodeBomb(b);
    ...
    if(b.life>0&&targets.length){if(b.bomb){b.x=targets[0].x;explodeBomb(b);}else hitEnemy(targets[0],CONFIG.bulletDamage,b.dir);b.life=0;}
  }
  if(state==='playing'){updateThunderBullet(dt);updateDirtBalls(dt);if(selectedCharacter==='tetsu')updateTetsu(dt);else if(selectedCharacter==='denden')updateSkills(dt);updateNyoro(dt);updateBombs(dt);updateMira(dt);}
...
function drawPlayer(){ctx.save();if(player?.purple>0&&Math.floor(elapsed*20)%2)ctx.filter='sepia(1) saturate(5) hue-rotate(220deg)';drawPlayerBody();ctx.restore();}
function drawPlayerBody(){if(selectedCharacter==='miramob'){paintMira(player,miraAction,MIRA.playerHeight);return;}if(selectedCharacter==='nyoro'){drawNyoro();return;}if(selectedCharacter==='denden'&&drawDendenBulletPose())return;if(selectedCharacter==='tetsu'){drawTetsu();return;}const p=player;if(!p)return;const img=playerImage();if(!img)return;ctx.save();ctx.fillStyle='#173b3b40';ctx.beginPath();ctx.ellipse(p.x-camera,p.y+1,29,6,0,0,Math.PI*2);ctx.fill();ctx.translate(p.x-camera,p.y);ctx.scale(p.dir,1);if(p.inv>0&&Math.floor(p.inv*16)%2===0)ctx.globalAlpha=.35;const h=CONFIG.playerHeight,w=img.width/img.height*h;
...
function drawSkills(){drawNyoroEffects();drawBombs();if(selectedCharacter==='nyoro'||selectedCharacter==='miramob')return;if(selectedCharacter==='tetsu'){drawTetsuEffects();return;}const s=skillState,p=player;
  ...
  for(const ex of explosions)drawBubbleExplosion(ex);
  drawThunderBullet();drawGiantThunder();
...
$('pink-tap').onclick=commandPink;
window.addEventListener('keydown',e=>{if(e.target instanceof Element&&e.target.closest('input,textarea,select'))return;if(e.code==='KeyR'&&!e.repeat){e.preventDefault();commandPink();}});

$('admin-form').addEventListener('submit',e=>{e.preventDefault();if(state==='playing'||state==='paused')return;if($('admin-code').value==='0329'){adminUnlocked=true;document.querySelector('[data-character="miramob"]').hidden=false;$('admin-status').textContent='ミラモブを選択できるようになりました';}else $('admin-status').textContent='コードが違います';$('admin-code').value='';});
