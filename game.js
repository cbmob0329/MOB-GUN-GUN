...
function tick(dt){
  ...
  for(const e of enemies){e.flash=Math.max(0,e.flash-dt);if(e.launch){const f=e.launch;e.x=clamp(e.x+f.vx*dt,24,CONFIG.worldWidth-24);e.y+=f.vy*dt;f.vy+=CONFIG.gravity*dt;f.vx*=Math.exp(-1.3*dt);f.angle+=f.spin*dt;if(e.death>=0)e.death+=dt;if(e.y>=f.floor&&f.vy>0){e.y=f.floor;e.home=e.x;e.launch=null;}continue;}if(e.death>=0){e.death+=dt;continue;}if(Math.abs(e.x-p.x)>1000)continue;const c=CONFIG.enemies[e.type];if(e.type==='chase'&&Math.abs(e.x-p.x)<470)e.dir=Math.sign(p.x-e.x)||e.dir;else if(e.x<e.home-e.range)e.dir=1;else if(e.x>e.home+e.range)e.dir=-1;e.x+=e.dir*c.speed*dt+e.knock*dt;e.knock*=Math.exp(-10*dt);e.x=clamp(e.x,e.home-e.range-80,e.home+e.range+80);if(Math.abs(p.x-e.x)<half+e.w*.42&&p.y>e.y-e.h&&p.y-CONFIG.playerColliderHeight<e.y)damagePlayer(e);}
  ...
  if(state==='playing'){if(selectedCharacter==='tetsu')updateTetsu(dt);else updateSkills(dt);}
  bullets=bullets.filter(b=>b.life>0&&b.x>camera-240&&b.x<camera+W+240);enemies=enemies.filter(e=>e.death<.6||e.launch);
