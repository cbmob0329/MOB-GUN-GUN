...
async function main(){
 ...
 try{for(const mobile of [false,true]){
  ...
  const results=await page.evaluate(()=>{
   ...
   const setup=(character='denden')=>{selectedCharacter=character;reset();pink.enabled=false;state='playing';enemies=[];updateCharacterUI();};
   setup();keys.add('KeyJ');let shots=0,lastAge=10,reloads=0;for(let i=0;i<175;i++){tick(1/120);if(player.shotAge<lastAge)shots++;lastAge=player.shotAge;}check(shots===8&&player.ammo===0&&player.reload>0,'eight rounds then reload');keys.clear();const reload=player.reload;pause();step(120);check(player.reload===reload,'pause reload');resume();step(120);check(player.ammo===8&&player.reload===0,'automatic one second reload while released');check(CONFIG.shootInterval===.2,'slightly slower cadence');
   ...
   const frozen=a.age;pause();step(80);check(a.age===frozen,'arena pauses');resume();player.inv=100;const sides=new Set();for(let i=0;i<1800&&a.state!=='cleared';i++){for(const enemy of enemies){if(enemy.arenaId===a.id){sides.add(enemy.dir);hitEnemy(enemy,1000);}}tick(1/120);}check(a.state==='cleared'&&a.spawned===48&&sides.size===2,'three waves from both sides then unlock');player.x=a.right-70;keys.add('KeyD');step(90);check(player.x>a.right,'exit after all waves');
   ...
   return {ammo:8,reload:1,shotInterval:.2,combo:'12 + 16',heal:8,waves:3,arenaEnemies:48};
  ...
  if(!mobile){for(const [name,x] of [['bridge',3500],['slope',4900],['trampoline',6420],['crumble',7500],['arena',9750]]){await page.evaluate(x=>{reset();pink.enabled=false;state='playing';player.x=x;player.inv=100;camera=x-300;for(let i=0;i<100;i++)tick(1/120);},x);await page.screenshot({path:path.join(process.env.TEMP,`world-${name}.png`)});}}
