...
function thunderArea(x,y,r,damage,stun){for(const e of combatTargets()){if(e.death>=0||Math.abs(e.x-x)>r+e.w/2||e.y<y-150||e.y-e.h>y+20)continue;hitEnemy(e,damage,Math.sign(e.x-x)||player.dir);if(e.type!=='crate'){e.electric=Math.max(e.electric||0,stun);e.stun=Math.max(e.stun||0,e.type==='miramob'?Math.min(.25,stun):stun);e.attackAge=-1;}}}
